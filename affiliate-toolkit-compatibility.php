    ...
    class atkp_compatibility
    {
        ...
        function admin_menu() {
            ...
            add_submenu_page(
                    ...
                    __('Compatibility', ATKP_PLUGIN_PREFIX),
                    __('Compatibility', ATKP_PLUGIN_PREFIX),            
                    'manage_options',            
...
public function toolkit_compatibility() {
   ...
   foreach(atkp_compatibility::$modes as $key => $value){
    $class = ( $tab == $current ) ? ' nav-tab-active' : '';
        ...
        ?> <a class="nav-tab<?php echo $class ?>" href="<?php echo admin_url() ?>admin.php?page=<?php echo ATKP_PLUGIN_PREFIX.'_affiliate_toolkit-compatibility' ?>&tab=<?php echo $current ?>"><?php echo $key ?></a><?php
    $current = $current +1;
   }
   ?>
...
$current = 1;
 ...
 <div class="atkp-credits">
					<h3 class="hndle"><?php echo __('Affiliate Toolkit Starter', ATKP_PLUGIN_PREFIX) . ' '. ATKPSettings::plugin_get_version() ?></h3>
					<div class="inside">
    ...
    <div style="text-align:center;width:1024px;margin-top:10px" > <?php ATKPHomeLinks::echo_banner(); ?></div>
 <?php
